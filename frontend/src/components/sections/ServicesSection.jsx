                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1.5rem" }}>
                    {SERVICES.map((s, i) => (
                        <div key={s.title} className="service-card" style={{ animationDelay: `${i * 0.1}s` }}>
                            <div style={{
                                width: 52, height: 52, borderRadius: 14, background: s.color + "15",
                            ...
                            <h3 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "1.1rem", marginBottom: "0.6rem", letterSpacing: "-0.3px" }}>{s.title}</h3>
                            <p style={{ color: "#7A7A9A", fontSize: "0.88rem", lineHeight: 1.7, marginBottom: "1.2rem", fontWeight: 300 }}>{s.desc}</p>
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                <span style={{ color: "#FF6B2B", fontSize: "0.9rem", fontWeight: 600 }}>{s.price}</span>
                                <span style={{ fontSize: "0.8rem", color: "#5A5A7A", transition: "color 0.3s" }}>Xem chi tiết →</span>
                            </div>
                        </div>
                    ))}
                </div>
